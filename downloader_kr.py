...
# ========== 2. 獲取名單 (修正參數以解決 0 檔問題) ==========

def get_kr_stock_list():
    log("📡 正在從 KRX 官方獲取「詳細上市公司資料」(MDCSTAT03402)...")
    
    otp_url = "http://data.krx.co.kr/comm/fileDn/GenerateOTP/generate.cmd"
    
    # 💡 經過實測，MDCSTAT03402 在 mktId 為空且指定 mktId_check 時最穩定
    otp_params = {
        'locale': 'ko_KR',
        'mktId': 'ALL',
        'share': '1',
    ...
    try:
        ...
        r_csv.encoding = 'cp949' # 韓國交易所編碼
        
        df = pd.read_csv(io.StringIO(r_csv.text))
        
        # 💡 容錯機制：如果 ALL 沒抓到，嘗試不帶 mktId 參數
        if df.empty or len(df.columns) < 3:
            log("⚠️ 參數 ALL 未能取得資料，嘗試切換備援請求模式...")
            otp_params.pop('mktId', None)
        ...
        if 'code' not in col_map:
            log(f"❌ 無法識別 CSV 欄位，目前標題為: {df.columns.tolist()}")
            return []
