# -*- coding: utf-8 -*-
import os, io, time, random, sqlite3, requests, re
import pandas as pd
from datetime import datetime
# 💡 導入你現有的通知工具
from notifier import StockNotifier

# ========== 1. 環境設定 ==========
MARKET_CODE = "kr-share"
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(BASE_DIR, "kr_stock_warehouse.db")

def log(msg: str):
    print(f"{pd.Timestamp.now():%H:%M:%S}: {msg}")

# ========== 2. 數據偵察函式 (發送到 Telegram) ==========

def debug_to_telegram(df):
    """將抓到的檔案結構送往 Telegram 診斷"""
    notifier = StockNotifier()
    
    # 1. 取得所有欄位名稱
    cols = " | ".join(df.columns.tolist())
    
    # 2. 取得前 5 筆範例數據 (縮短長度以免 TG 訊息過長)
    sample_rows = ""
    for i in range(min(5, len(df))):
        row_values = [str(v)[:15] for v in df.iloc[i].values] # 每格最多取 15 字
        sample_rows += f"Row {i+1}: {' | '.join(row_values)}\n\n"
    
    msg = (
        f"🇰🇷 <b>KRX 原始檔案偵察報告</b>\n\n"
        f"<b>全部欄位:</b>\n<code>{cols}</code>\n\n"
        f"<b>前 5 筆數據範例:</b>\n<pre>{sample_rows}</pre>"
    )
    
    log("📤 正在發送原始數據結構至 Telegram...")
    notifier.send_telegram(msg)

# ========== 3. 獲取韓國清單 (修改版) ==========

def get_kr_stock_list():
    """從 KRX 官方獲取清單並執行 Telegram 診斷"""
    log("📡 正在從 KRX 官方獲取原始檔案...")
    
    otp_url = "http://data.krx.co.kr/comm/fileDn/GenerateOTP/generate.cmd"
    otp_params = {
        'locale': 'ko_KR',
        'mktId': 'ALL',
        'share': '1',
        'csvxls_isNo': 'false',
        'name': 'fileDown',
        'url': 'dbms/MDC/STAT/standard/MDCSTAT01901'
    }
    
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'Referer': 'http://data.krx.co.kr/'
    }
    
    try:
        r_otp = requests.post(otp_url, data=otp_params, headers=headers, timeout=15)
        otp_code = r_otp.text
        
        dn_url = "http://data.krx.co.kr/comm/fileDn/download_csv/download.cmd"
        r_csv = requests.post(dn_url, data={'code': otp_code}, headers=headers, timeout=30)
        r_csv.encoding = 'cp949'
        
        df = pd.read_csv(io.StringIO(r_csv.text))
        
        # 🔥 核心動作：發送診斷訊息
        debug_to_telegram(df)
        
        # 這裡之後的邏輯先保持原樣，或者你可以先 return 空，等看過 TG 訊息再動
        log("✅ 診斷訊息已送出，請檢查 Telegram 頻道。")
        
        # 暫時為了測試，我們還是跑完基本的映射邏輯
        col_map = {}
        for col in df.columns:
            c = str(col).strip()
            if '단축코드' in c: col_map['code'] = col
            elif '한글 종목약명' in c: col_map['name'] = col
            elif '시장구분' in c: col_map['market'] = col
            elif '업종명' in c: col_map['sector'] = col

        # 這裡之後的存檔邏輯你可以先不用動，直接執行這個函式即可
        return [] 

    except Exception as e:
        log(f"❌ 偵察失敗: {e}")
        return []

if __name__ == "__main__":
    # 直接執行測試
    get_kr_stock_list()
